...
def databaseToDataShop(path, classinfo=None):
  ...
  """
    ...
    with open('output-072616.xml', 'a') as the_file:
...
<tutor_related_message_sequence version_number="4">\n"""
            ...
            if row[colmap['speaker']] == 'system':
                ...
                if tkclist is not None:
                    ...
                    for kc_row in kc_res:
                        tskill = tskill + "\n\t\t\t<skill><name>%s</name><category>tutor turn</category></skill>" % \
                                          kc_row[0]
    ...
    # print result  # to capture the intended output-072616
...
def runsc(path):
  ...
  Returns a string with sc's output-072616 to STDERR,
