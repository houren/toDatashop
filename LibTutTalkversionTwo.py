...
<tutor_related_message_sequence version_number="4">\n"""
    ...
    """ % locals()
            ...
            #evaluate action, i.e. correct/incorrect
            ae = ''
            ...
            #update step name for event descriptor
            select = ''
            act = ''
            ...
            #update semantic event time based on user turn, i.e. teacher or student

            if row[colmap['speaker']] == 'system':
                semev = '\n\t\t\t<semantic_event transaction_id="%s" name="Tutor Turn" />' %(ruuid)
            else:
                semev = '\n\t\t\t<semantic_event transaction_id="%s" name="RESPONSE" />' % (ruuid)
                tutorfeed = nextrow[colmap['string']]
                if tutorfeed is not None:
                    tutorfeed = cgi.escape(tutorfeed)
                ta = "\n\t\t\t<tutor_advice><![CDATA[%s]]></tutor_advice>" % tutorfeed
